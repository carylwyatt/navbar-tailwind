      ...
      <div :class="isOpen ? 'block' : 'hidden'" class="lg:flex lg:items-center">
          <div 
            ...
            <div :class="{active:index == selected}" class="lg:absolute lg:left-0 dropdown-content flex flex-col bg-white lg:mt-20 hidden">
              <nuxt-link v-for="link in item.links" :key="link.title" class="px-4 py-2  text-red-800 hover:bg-gray-200" to="/">
                <font-awesome-icon :icon="[link.iconClass, link.icon]" class="fa-fw fill-current" /> 
                  <h4>{{ link.title }}</h4>
